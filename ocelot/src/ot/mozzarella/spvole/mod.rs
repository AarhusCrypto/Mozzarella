...
mod tests {
    use super::{
        prover::{BatchedProver, Prover, SingleProver},
        verifier::{BatchedVerifier, SingleVerifier, Verifier},
    };
    use crate::ot::{
        ...
        for test_i in 0..TEST_REPETITIONS {
            ...
            let prover_thread = spawn(move || {
                ...
                sp_prover
                    ...
                    .unwrap();
                (out_u, out_w, sp_prover.get_alpha())
            });
            ...
            let (out_u, out_w, alpha) = prover_thread.join().unwrap();
